...
import type { DashboardSummary, PaginatedResponse, RowsResponse, TickerRow } from './dashboard.types';
...
export class SymbolSnapshotService {
  ...
  async getAllRows(): Promise<RowsResponse<TickerRow> | null> {
    ...
    const updatedAt = rows[0]?.updatedAt.toISOString() ?? (await this.getLatestUpdatedAt()) ?? new Date().toISOString();
    return {
  ...
  async getSummary(): Promise<DashboardSummary | null> {
    if (!this.isEnabled()) return null;
    const [activeTickers, aboveEma20, aboveEma50, aboveEma200, rsScoreGt70, unusualVolToday, qualityCandidates] = await this.prisma.$transaction([
      ...
      this.prisma.symbolSnapshot.count({ where: { OR: [{ unusualVol: true }, { volDevPct5: { gte: 50 } }] } }),
  ...
  private buildWhere(filters: {
    ...
    if (filters.symbols?.length) where.symbol = { in: filters.symbols.map((item) => item.toUpperCase()) };
    ...
    if (Number.isFinite(filters.minCap) && (filters.minCap ?? 0) > 0) where.marketCap = { gte: filters.minCap! };
    if (filters.excludeNear52w) where.near52wHigh = false;
    if (Number.isFinite(filters.rsMin)) where.rsScore = { ...(where.rsScore as object || {}), gte: filters.rsMin! };
    if (Number.isFinite(filters.volRelMax)) where.volRelative = { lte: filters.volRelMax! + 1e-9 };
  ...
  async getSemaphorePage(params: {
    page?: number; pageSize?: number; sector?: string[]; industry?: string[]; minCap?: number; excludeNear52w?: boolean; symbols?: string[]; sort?: string; country?: string[]; indexTag?: string[];
    ...
    const orderBy: Prisma.SymbolSnapshotOrderByWithRelationInput[] =
      params.sort === 'change' ? [{ changePct: 'desc' }, { symbol: 'asc' }] :
      params.sort === 'sym' ? [{ symbol: 'asc' }] : [{ rsScore: 'desc' }, { symbol: 'asc' }];
    ...
    const updatedAtRow = rows[0] ?? (await this.prisma.symbolSnapshot.findFirst({ orderBy: { updatedAt: 'desc' } }));
    return {
      items: rows.map(toTickerRow),
      ...
      total,
      totalPages,
      updatedAt: updatedAtRow?.updatedAt.toISOString() ?? new Date().toISOString(),
    };
  }

  async getWarrenPage(params: {
    page?: number; pageSize?: number; sector?: string[]; industry?: string[]; minCap?: number; excludeNear52w?: boolean; symbols?: string[]; country?: string[]; indexTag?: string[];
  }): Promise<PaginatedResponse<TickerRow> | null> {
    if (!this.isEnabled()) return null;
    const safePage = normalizePage(params.page);
    const safePageSize = normalizePageSize(params.pageSize);
    ...
    const [total, rows] = await this.prisma.$transaction([
      ...
      this.prisma.symbolSnapshot.findMany({
        where,
        orderBy: [{ warrenScore: 'desc' }, { rsScore: 'desc' }, { symbol: 'asc' }],
        skip: (safePage - 1) * safePageSize,
        take: safePageSize,
      }),
    ]);
  ...
  async getRsTrendRows(params: {
    country?: string[]; indexTag?: string[]; sector?: string[]; industry?: string[]; minCap?: number; excludeNear52w?: boolean; rsMin?: number; limit?: number; symbols?: string[];
  }): Promise<RowsResponse<TickerRow> | null> {
    if (!this.isEnabled()) return null;
    const limit = Math.max(1, Math.min(100, Number(params.limit) || 20));
    const where = this.buildWhere(params);
    if (Number.isFinite(params.rsMin)) where.rsScore = { ...(where.rsScore as object || {}), gte: params.rsMin! };
    const rows = await this.prisma.symbolSnapshot.findMany({
      where,
  ...
  async getVolatilityRelativeRows(params: {
    country?: string[]; indexTag?: string[]; sector?: string[]; industry?: string[]; minCap?: number; excludeNear52w?: boolean; rsMin?: number; limit?: number; symbols?: string[]; sort?: 'asc' | 'desc';
  }): Promise<RowsResponse<TickerRow> | null> {
    if (!this.isEnabled()) return null;
  ...
  async getScannerPage(params: {
    page?: number; pageSize?: number; country?: string[]; indexTag?: string[]; sector?: string[]; industry?: string[]; minCap?: number; excludeNear52w?: boolean; symbols?: string[]; rsMin?: number; volRelMax?: number; rsiMin?: number; rsiMax?: number; onlyUnusualVol?: boolean;
  }): Promise<PaginatedResponse<TickerRow> | null> {
    if (!this.isEnabled()) return null;
    const safePage = normalizePage(params.page);
    const safePageSize = normalizePageSize(params.pageSize);
