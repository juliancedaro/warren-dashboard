...
function RsiRsTooltip({ active, payload }: RechartsTooltipProps<DashboardRow>) {
      ...
      <div className="dash-chart-wrap dash-chart-wrap-short dash-chart-recharts">
        {loading && rows.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%" minHeight={280}>
            <ScatterChart margin={{ top: 12, right: DASH_RECHARTS_MARGIN_RIGHT, bottom: 8, left: 10 }}>
              <CartesianGrid stroke="var(--dash-grid)" strokeDasharray="4 6" />
              {/* Eje X un poco más allá de 100 para que los puntos no queden pegados al borde derecho */}
              <XAxis
                type="number"
                dataKey="rsScore"
                name="RS"
                ...
                label={{ value: 'RS Score', position: 'bottom', offset: 0, fill: 'var(--dash-muted)', fontSize: 11 }}
              />
              ...
              <Tooltip cursor={false} content={<RsiRsTooltip />} />
              <Scatter name="Tickers" data={rows}>
                {rows.map((row) => <Cell key={row.symbol} fill={rsiRsFill(row)} />)}
              </Scatter>
            </ScatterChart>
          </ResponsiveContainer>
        )}
